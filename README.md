...
```bash
./build.sh --pkg
sudo installer -pkg build/GoogleHomeCameraWidget-0.1.4.pkg -target /
...
2. Install `build/GoogleHomeCameraWidget-0.1.4.pkg` for the system `/Applications` install that registers the desktop widget.
